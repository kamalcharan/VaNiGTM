...
export function ConsoleNav({ active, isOwner }: { active: 'leads' | 'partners'; isOwner: boolean }) {
  ...
  return (
    <div className={s.cNav}>
      <a className={active === 'leads' ? s.cNavOn : ''} onClick={() => router.push('/gtm/audience/people')}>Leads</a>
      {/* Partner links are an owner-only screen — a partner has exactly one
          ...
          control. */}
      {isOwner && (
        <a className={active === 'partners' ? s.cNavOn : ''} onClick={() => router.push('/gtm/audience/people/partners')}>
          Partner links
        </a>
      )}
      <span className={s.who}>
        ...
        <a onClick={() => { logout(); router.push('/gtm/audience/people/login'); }}>Sign out</a>
      </span>
    </div>
  );
}
