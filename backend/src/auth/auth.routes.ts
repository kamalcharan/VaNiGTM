...
export function extractJwt(req: { headers: { authorization?: string } }): JwtPayload | null {
...
/* ── Onboarding ─────────────────────────────────────── */
/*
 * Moved to src/onboarding/onboarding.routes.ts.
 *
 * Onboarding is an agent now: one engine that runs a declared lane, for the
 * product tier and for each agent's activation. The version that lived here
 * could only ever run the two steps register() seeds, and updated a row that
