-- ============================================================
-- 048_ki_txn_import_customer_lookup.sql
--
-- Two fixes to match kewalinvest transaction import behaviour:
--
-- 1. customer_lookup_method on ki_import_sessions
--    Kewalinvest lets the user choose how to match CSV rows to
--    clients: 'iwell_code', 'customer_name', or 'both'.
--    Without this, the RPC had no user control and always tried
--    all three — which caused silent mismatches.
--
-- 2. ISIN scheme lookup in ki_process_txn_import_session
--    CAS / Tbook CSVs carry ISIN codes. The RPC should resolve
--    the scheme via ISIN (same as bookmark import) BEFORE
--    falling back to alias name lookup.  This fixes schemes
--    whose nav_name is not yet seeded in ki_scheme_aliases.
--
--    Full lookup chain after this migration:
--      a) ISIN → ki_schemes (isin_growth / isin_dividend)
--      b) scheme_name → ki_scheme_aliases (global, indexed)
--      c) scheme_name → ki_scheme_bookmarks.alias_name (fallback)
-- ============================================================

BEGIN;

-- ─────────────────────────────────────────────────────────────────────────────
-- 1. Add customer_lookup_method to ki_import_sessions
-- ─────────────────────────────────────────────────────────────────────────────
ALTER TABLE ki_import_sessions
    ADD COLUMN IF NOT EXISTS customer_lookup_method TEXT
        NOT NULL DEFAULT 'iwell_code'
        CHECK (customer_lookup_method IN ('iwell_code', 'customer_name', 'both'));

DO $$ BEGIN
    RAISE NOTICE '[048] ki_import_sessions.customer_lookup_method: column added';
END $$;

-- ─────────────────────────────────────────────────────────────────────────────
-- 2. Updated ki_process_txn_import_session
--    - New param: p_customer_lookup_method (default 'iwell_code')
--    - STEP 1 now uses kewalinvest's exact 3-mode client lookup
--    - STEP 3 now tries ISIN first, then alias, then bookmark
-- ─────────────────────────────────────────────────────────────────────────────
...
BEGIN
    ...
    FOR v_staging IN (
        ...
        BEGIN
            ...
            --
            -- Matches kewalinvest's process_transaction_import_session exactly:
            ...
            --
            -- Path A: ISIN → ki_schemes (most reliable — ISIN is unique)
            -- Path B: scheme_name → ki_scheme_aliases (global, indexed)
            -- Path C: scheme_name → ki_scheme_bookmarks.alias_name (fallback)
            --
            -- If resolved, scheme must be bookmarked by this tenant.
...
DO $$ BEGIN
    RAISE NOTICE '[048] ki_import_sessions.customer_lookup_method: added (iwell_code/customer_name/both)';
    RAISE NOTICE '[048] ki_process_txn_import_session(): updated — ISIN scheme lookup + 3-mode client lookup';
END $$;
