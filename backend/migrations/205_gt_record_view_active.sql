-- ============================================================
-- Migration: 205_gt_record_view_active.sql
...
-- ── SUPERSEDES 204 ────────────────────────────────────────────────────
--
-- 204 filtered `is_active = true` inside the view, which made deactivated
-- records unreachable rather than hidden — there was no way to show them.
-- The flag is now a COLUMN and the caller decides. Pool source rows have no
-- such flag: a delivery is retired at the load, not per row, so they are
-- always active.
-- ============================================================
...
SELECT
    ...
    true                        AS is_active,     -- retired at the load, not the row
