-- ============================================================================
-- Migration 057: Rename ki_alerts → ki_pulses + add follow-up tracking
--
-- Changes:
--   1. Rename table ki_alerts → ki_pulses
--   2. Rename column alert_type → pulse_type; widen CHECK to include follow-up types
--   3. Add follow-up columns: contact_id, origin, status, due_date,
--      snoozed_until, notes, snapshot_id, assigned_to, completed_at, completed_by
--   4. Migrate existing data: dismissed/acted_on → status column; origin = 'system'
--   5. Drop old dismissed/acted_on columns
--   6. Rebuild indexes and RLS policy under new name
--   7. CREATE OR REPLACE ki_process_txn_import_session to reference ki_pulses
-- ============================================================================
...
-- ── 2. RENAME alert_type → pulse_type ────────────────────────────────────────

ALTER TABLE ki_pulses RENAME COLUMN alert_type TO pulse_type;

-- Drop old CHECK constraint on pulse_type (was named ki_alerts_alert_type_check)
DO $$
...
ALTER TABLE ki_pulses
    ...
    CHECK (pulse_type IN (
        -- System-generated (kept from ki_alerts)
...
-- All existing rows were system-generated alerts
UPDATE ki_pulses SET origin = 'system';

-- Map dismissed/acted_on → status
UPDATE ki_pulses
SET    status = CASE
...
COMMENT ON COLUMN ki_pulses.pulse_type IS
    'System types: rebalance_needed | sip_at_risk | goal_behind | tax_harvest_opportunity | '
    'review_due | large_redemption | new_nfo_match | sip_bounced | nav_drop | new_scheme_detected. '
    'Manual types: prospect_followup | client_followup.';
...
COMMENT ON COLUMN ki_pulses.snapshot_id IS 'Optional link to ki_contact_snapshots — snapshot that triggered this follow-up.';
COMMENT ON COLUMN ki_pulses.assigned_to IS 'UUID of the vn_users row responsible for actioning this pulse.';

-- ── 8. RECREATE ki_process_txn_import_session (reference ki_pulses) ──────────
-- This is CREATE OR REPLACE — it supersedes all previous versions (045-055).
-- Only change from migration 055: ki_alerts → ki_pulses, alert_type → pulse_type.
-- ─────────────────────────────────────────────────────────────────────────────
...
BEGIN
    ...
    FOR v_staging IN (
        ...
        BEGIN
            ...
            -- One system pulse per new scheme per client (ki_pulses, not ki_alerts)
...
COMMENT ON FUNCTION ki_process_txn_import_session(INTEGER, TEXT) IS
    ...
    'Returns summary counts. Updated in migration 057 to reference ki_pulses.';
