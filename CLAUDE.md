...
15. **`ki_alerts` will be renamed — future feature called "Pulses".** The `ki_alerts` table is the current store for all system-generated and user-facing alerts (new scheme detected on import, JTBD nudges, etc.). A future product feature called **Pulses** will replace/extend this with richer alert types and UI. When that feature is built, the table may be renamed to `ki_pulses`. Until then, all alert writes go to `ki_alerts`. Do NOT reference the old name in new code — always use `ki_alerts`. Track this when the Pulses feature is scoped.